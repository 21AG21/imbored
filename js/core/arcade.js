...
(function (global) {
  ...
  /* [/] recomputing its target from currentGame every keydown looks safe but
     isn't: currentGame only updates once route() runs on the async
     'hashchange' event, so a burst of presses faster than that round trip
     (OS key-repeat easily manages it) all read the same stale currentGame,
     all compute the same target, and the browser no-ops every repeat
     location.hash assignment after the first — silently dropping the rest
     of the burst. Track the in-flight target here instead, so consecutive
     presses accumulate even before routing has caught up; route() resyncs
     it to ground truth once it actually runs. */
  let pendingGameIdx = null;
  let bossOn = false;
  ...
  Arcade.start = function start() {
    ...
    addEventListener('keydown', (e) => {
      ...
      if (bossOn) return;
      /* [ and ] never appear in any game's answer text (number guesses, word
         guesses, math answers, or the fixed typing-test passages), so unlike
         every other shortcut here they don't need the typing guard below —
         landing on one of the handful of games that autofocus a text input
         shouldn't silently swallow a shelf-switch shortcut with no visible
         sign why. Checked ahead of the typing guard, same as the panic key. */
      if (currentGame && (e.key === '[' || e.key === ']')) {
        e.preventDefault();
        const i = pendingGameIdx != null ? pendingGameIdx : games.indexOf(currentGame);
        if (i >= 0) {
          const j = (i + (e.key === ']' ? 1 : -1) + games.length) % games.length;
          pendingGameIdx = j;
          Arcade.go(games[j].id);
        }
        return;
      }
      if (typing) {
      ...
      /* in a game: R restarts it (skipped where letters are the controls) —
         [ and ] are handled above, ahead of the typing guard */
      if (currentGame) {
