...
(function (global) {
  ...
  /* Confetti removed: the old per-call version spawned a fresh full-screen
     canvas + resize listener + rAF loop every time, so firing it on every new
     record stacked overlays until the tab died. Celebrations use toasts and a
     sound now; the engine's shared singleton (Engine.fx) stays for the safe,
     bounded level-complete burst. */
  ...
  const WORDS = {
    matrix: () => matrix(),
    party: () => { if (A && A.great) A.great(); toast({ icon: '🎉', title: 'Party mode', body: 'You found the party. Keep it down.' }); },
  ...
  function wireLogo() {
    ...
    mark.addEventListener('click', () => {
      ...
      if (n >= 7) { n = 0; if (A && A.great) A.great(); toast({ icon: '🎉', title: 'You found it', body: 'Seven clicks on the logo. Impeccable procrastination.' }); }
    });
  }
